...
import (
	...
	natscl "github.com/a5bbbbb/AITUmoment/email_service/pkg/nats"
	"github.com/a5bbbbb/AITUmoment/email_service/pkg/safe"
...
func (c *PubSub) Stop() {
	...
	for _, sub := range c.subs {
		err := sub.Unsubscribe()
		if err != nil {
			logger.GetLogger().Println("failed to unsubscribe",
				"subject:", sub.Subject,
				"error:", err,
			)
		}
...
func (c *PubSub) consume(cfg PubSubSubscriptionConfig) error {
	...
	logger.GetLogger().Println("consuming NATS subject started", "subject: ", cfg.Subject)
