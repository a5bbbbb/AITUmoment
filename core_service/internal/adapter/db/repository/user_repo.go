...
func NewUserRepository() *UserRepository {
	logger.GetLogger().Trace("Creating user repo")
...
func (r *UserRepository) CreateUser(user *models.User) (int64, error) {
	var insertedID int
	err := r.db.QueryRow(
		"INSERT INTO users (username, educational_program, email, password, public_name, bio, group_id, verified) VALUES ($1, $2, $3, $4, $5, $6, $7, false) RETURNING id",
		user.Name, user.EducationalProgram, user.Email, user.Passwd, user.PublicName, user.Bio, user.Group,
	).Scan(&insertedID)
...
func (r *UserRepository) UpdateUserVerified(email string, verified bool) (*int, error) {
	query := ` 
	UPDATE users 
		SET verified = $1
	WHERE email = $2 
	...
	`
	var updatedId int
	err := r.db.QueryRow(query, verified, email).Scan(&updatedId)
	if err != nil {
		return nil, err
	}
