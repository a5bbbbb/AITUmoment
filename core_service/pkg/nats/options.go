...
func setOptions(ctx context.Context, hosts []string, nkey string, isTest bool) (nats.Options, error) {
	...
	opts.ReconnectedCB = func(nc *nats.Conn) {
		logger.GetLogger().Println(ctx, "nats reconnect status", "status", nc.Status().String())
	}

	opts.DisconnectedErrCB = func(nc *nats.Conn, err error) {
		logger.GetLogger().Println(ctx, "nats disconnect status", "status", nc.Status().String())

		if err != nil {
			logger.GetLogger().Println(ctx, "nats disconnect error", err.Error())
		}
	}

	opts.ClosedCB = func(nc *nats.Conn) {
		logger.GetLogger().Println(ctx, "nats closed status",
			"status", nc.Status().String())
	}
